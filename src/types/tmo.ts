...
export interface TMOLoginResponse {
  token: string;
  message?: string;
}
...
export interface TMOResetPasswordRequest {
  mobile: string;
  code: string;
  password: string;
  prefix: string;
...
export interface TMOChangePasswordRequest {
  oldPassword: string;
  newPassword: string;
}
...
export interface TMOCustomer {
  ...
  extension_attributes?: TMOCustomerExtensionAttributes;
  custom_attributes?: TMOCustomAttribute[];
  dob?: string;
}

export interface TMOCustomerExtensionAttributes {
...
export interface TMOCheckoutAddRequest {
  cartId: string;
  itemIds: number[];
}
...
export interface TMOAlipayQRCodeResponse {
  qr_code: string;
  order_id: string;
}
...
export interface TMOAPIError {
  message: string;
  errors?: TMOAPIErrorDetail[];
  code?: number;
  parameters?: Record<string, string>;
  trace?: string;
}

export interface TMOAPIErrorDetail {
  message: string;
  parameters?: Record<string, string>;
}

// ==================== Mapped Types for App ====================

// Map TMO Customer to existing User type format
...
// Map TMO Product to display format
export interface MappedProduct {
  ...
  commercial_credits?: number;
  category: string;
  categoryIds: string[]; // All category IDs this product belongs to
  url: string;
  images: string[];
  is_pack: boolean;
...
// Map TMO Cart Item to display format
...
// Map TMO Order to display format
...
// ==================== Geo Types (China only) ====================

// API response format for geo endpoints (CN only)
...
export type TMOGeoRegion = TMOGeoItem;  // Province/Region
export type TMOGeoCity = TMOGeoItem;     // City
export type TMOGeoDistrict = TMOGeoItem; // District
